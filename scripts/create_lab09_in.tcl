...
set out_dir [file normalize [file join $build_root lab09-in]]
set legacy_root [file join $repo_root third_party Digilent-ZYBO]
set source_project [file join $legacy_root Projects hdmi_in]
...
set work [file join $out_dir hdmi_in]
file copy -force $source_project $work

puts "ZYBO-LAB: rebuilding pinned Original ZYBO hdmi_in reference"
...
if {![regexp {^(2015|2016|2017)\.} $vivado_ver]} {
    puts "WARNING: hdmi_in is a pinned legacy backend exported by old Vivado."
...
if {$mode eq "bd"} {
    puts "ZYBO-LAB: hdmi_in block design validation complete"
    close_project
    exit 0
}
...
if {[file exists $bit_src]} { file copy -force $bit_src [file join $out_dir lab09-in.bit] }
