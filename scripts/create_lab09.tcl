...
set out_dir [file normalize [file join $build_root lab09]]
set legacy_root [file join $repo_root third_party Digilent-ZYBO]
set source_project [file join $legacy_root Projects hdmi_out]

if {![file exists [file join $source_project proj create_project.tcl]]} {
    error "Missing pinned Digilent HDMI reference. Run: make legacy-ip"
}
...
set work [file join $out_dir hdmi_out]
...
if {[llength $bd] == 0} {
    error "Legacy HDMI project did not create a block design"
}
validate_bd_design
save_bd_design
...
if {$mode eq "bd"} {
    puts "ZYBO-LAB: legacy HDMI block design validation complete"
    close_project
    exit 0
}
...
if {[get_property PROGRESS [get_runs impl_1]] ne "100%"} {
    error "Legacy HDMI implementation did not complete"
}
...
if {[file exists $bit_src]} {
    file copy -force $bit_src [file join $out_dir lab09.bit]
}

puts "ZYBO-LAB: legacy HDMI build complete: $out_dir"
close_project
